...
public class Chromo implements Comparable<Chromo> {
	...
	public int distanceTo(Chromo X) {
		int distance = 0;
		for (int i = 0; i < Parameters.numGenes; i++) {
			if (this.chromo.get(i) != X.chromo.get(i)) {
				distance++;
			}
		}
		return distance;

		//return (int)Math.abs(this.rawFitness - X.rawFitness);
	}
	...
	public static int selectParent() {
		...
		case 1: // Proportional Selection
			...
			for (j = 0; j < Parameters.popSize; j++) {
				rWheel = rWheel + Search.member.get(j).proFitness;
				if (randnum < rWheel)
					return (j);
			}
			break;
		...
		case 2: // Tournament Selection
			...
			for (int i = 3; i > 0; i--) {
				for (j = 0; j < i; j++) {
					if (Search.member.get(candidate[j]).sclFitness > Search.member.get(candidate[j + 1]).sclFitness) {
						temp = candidate[j];
